...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!broker) {
      return respond(400, { error: 'Missing required field: broker' });
    }
    ...
    if (action === 'get_auth_url') {
      ...
      if (!urlBuilder) {
        return respond(400, { error: `No OAuth URL builder for broker: ${broker}` });
      }
      const siteUrl = redirect_uri || process.env.URL || 'http://localhost:8888';
      const authUrl = urlBuilder(siteUrl + '/auth/callback');
      if (!authUrl) {
        return respond(400, {
          error: `API key not configured for ${broker}. Add the env variable in Netlify dashboard.`,
          needs_env: true,
        });
      }
      return respond(200, { broker, auth_url: authUrl });
    }
    ...
    return respond(500, { error: 'Auth error: ' + err.message });
  }
};
